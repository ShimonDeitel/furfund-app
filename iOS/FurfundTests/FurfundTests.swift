import XCTest
@testable import Furfund

@MainActor
final class FurfundTests: XCTestCase {
    func makeIsolatedStore() -> FurfundStore {
    ...
    func testSeedDataLoadsBelowFreeLimit() {
        ...
        XCTAssertLessThan(store.entries.count, FurfundStore.freeEntryLimit)
    }

    func testAddEntrySucceedsUnderLimit() {
        let store = makeIsolatedStore()
        let before = store.entries.count
        let added = store.addEntry(date: Date(), amount: 0, notes: "Test note", isPro: false)
        XCTAssertTrue(added)
    ...
    func testCanAddEntryRespectsFreeLimit() {
        let store = makeIsolatedStore()
        for _ in 0..<(FurfundStore.freeEntryLimit + 5) {
            _ = store.addEntry(date: Date(), amount: 0, notes: "Filler", isPro: false)
        }
        XCTAssertEqual(store.entries.count, FurfundStore.freeEntryLimit)
    ...
    func testProBypassesFreeLimit() {
        let store = makeIsolatedStore()
        for _ in 0..<(FurfundStore.freeEntryLimit + 5) {
            _ = store.addEntry(date: Date(), amount: 0, notes: "Filler", isPro: true)
        }
        XCTAssertGreaterThan(store.entries.count, FurfundStore.freeEntryLimit)
    }

    func testDeleteEntryRemovesIt() {
        let store = makeIsolatedStore()
        _ = store.addEntry(date: Date(), amount: 0, notes: "note", isPro: false)
    ...
    func testUpdateEntryChangesFields() {
        ...
        store.updateEntry(entry.id, date: entry.date, amount: 0, notes: entry.notes)
    ...
    func testEntriesSortedByDateDescending() {
        ...
        let newer = Date()
        _ = store.addEntry(date: older, amount: 0, notes: "note", isPro: false)
        _ = store.addEntry(date: newer, amount: 0, notes: "note", isPro: false)
