        ...
        NavigationStack {
            ZStack {
                ...
                List {
                    ...
                    ForEach(store.entries) { entry in
                        Button {
                            draft = EntryDraft(date: entry.date, amount: entry.amount, notes: entry.notes)
                            editingID = entry.id
                            showingAdd = true
                        } label: {
            ...
            .sheet(isPresented: $showingAdd) {
                AddEntrySheet(draft: $draft, isEditing: editingID != nil) {
                    if let id = editingID {
                        store.updateEntry(id, date: draft.date, amount: draft.amount, notes: draft.notes)
                    } else {
                        store.addEntry(date: draft.date, amount: draft.amount, notes: draft.notes, isPro: purchases.isPro)
                    }
                    showingAdd = false
                }
            }
